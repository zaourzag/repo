...
class Hits:
    ...
    def __init__(self, i, epg=False, event=False):
        self.item = {}
        self.epg = epg
        self.event = event
        self.type = i['type']
        ...
        self.start = utc2local(i.get('startDate', ''))
        self.end = utc2local(i.get('endDate', ''))
    ...
    def airing_item(self):
        ...
        start = plot_time(self.start, self.event)
        end = plot_time(self.end, self.event)
    ...
    def video(self, i):
        ...
        if self.appears and not self.start:
            self.start = utc2local(self.appears[:19] + 'Z')
        self.video_item()
