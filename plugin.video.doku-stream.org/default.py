...
BASE_URL = 'https://www.doku-stream.org/wp-json/wp/v2'
...
def parse_ids_content(content):
    ...
    match = soup.find('param', {'name': 'movie', 'value': re.compile('youtube')})
    if match:
        return match['value']
    match = soup.find('iframe', {'src': re.compile('youtube')})
    if match:
        return match['src']
