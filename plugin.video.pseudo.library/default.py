...
addnon_id = 'plugin.video.pseudo.library'
...
def fillPlugins(type='video'):
    ...
    for f in detail:
        ...
        if names and paths:
            ...
            elif type == 'audio' and path.startswith('plugin.audio') and not path.startswith('plugin.video.pseudo.library'):
                thumbnail = removeNonAscii(thumbnails.group(1))
                fanart = removeNonAscii(fanarts.group(1))
                addDir(name, 'plugin://' + path, 101, thumbnail, fanart, description, type, 'date', 'credits')
def addItem(labels="n.a"):
    if labels != 'n.a':    
        try:
            addon_log('addItem')
            u = "plugin://plugin.video.pseudo.library/?url=" + "&mode=" + str(5) + "&fanart=" + urllib.quote_plus(iconRemove)
            ok = True
