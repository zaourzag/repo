...
cookiePath = xbmc.translatePath(os.path.join(temp, "cookie.txt")).decode("utf-8")
...
def login():
    ...
    payload = {'form': 'UserLogin', 'loginUsername': user, 'loginPassword': pw}
...
def newthread(url):
    ...
    if "<!-- Deutsche Untertitel -->" in content or "<h2>Deutsche Untertitel</h2>" in content:
        ...
        folgede, untertitel_qualitaetde, untertitel_releasede, untertitel_linkde, lang_arrayde = get_content(contentDE,
                                                                                                             "de")
        ...
        if video['episode']:
            for folge_zeile in range(0, len(folgede), 1):
                ...
                if int(folgede[folge_zeile]) == int(video['episode']):
                    addLink("Staffel " + video['season'] + " Folge " + video['episode'] + " " + untertitel_releasede[
                        folge_zeile] + " ( " + untertitel_qualitaetde[folge_zeile] + " ) ",
                            untertitel_linkde[folge_zeile], "download", duration="", desc="", genre='',
                            lang=lang_arrayde[folge_zeile])
                    gefunden = 1
                    # Englisch
            ...
            for folge_zeile in range(0, len(folge), 1):
                try:
                    if int(folge[folge_zeile]) == int(video['episode']):
                        addLink("Staffel " + video['season'] + " Folge " + video['episode'] + " " + untertitel_release[
                            folge_zeile] + " ( " + untertitel_qualitaet[folge_zeile] + " ) ",
                                untertitel_link[folge_zeile], "download", duration="", desc="", genre='',
                                lang=lang_array[folge_zeile])
                        gefunden = 1
                except ValueError:
                    pass
    ...
    if gefunden == 0:
        for folgenr in range(0, len(folgede), 1):
            addLink(video['season'] + " Folge " + folgede[folgenr] + " " + untertitel_releasede[folgenr] + " ( " +
                    untertitel_qualitaetde[folgenr] + " ) ", untertitel_linkde[folgenr], "download", "", duration="",
                    desc="", genre='', lang=lang_arrayde[folgenr])
        for folgenr in range(0, len(folge), 1):
            addLink(video['season'] + " Folge " + folge[folgenr] + " " + untertitel_release[folgenr] + " ( " +
                    untertitel_qualitaet[folgenr] + " ) ", untertitel_link[folgenr], "download", "", duration="",
                    desc="", genre='', lang=lang_array[folgenr])
...
def get_content(content, lang):
    ...
    if folge_array and untertitel_qualitaet and untertitel_release_array and untertitel_link_array and untertitel_lang:
        folge_array, untertitel_qualitaet, untertitel_release_array, untertitel_link_array, untertitel_lang = (list(x)
                                                                                                               for x in
                                                                                                               zip(
                                                                                                                   *sorted(
                                                                                                                       zip(
...
def oldthread(url):
    ...
    if '<span style="font-size: 14pt"><strong><span style="text-decoration: underline">Englische Untertitel:</span></strong>' in content:
        contentEN = content[content.find(
            '<span style="font-size: 14pt"><strong><span style="text-decoration: underline">Englische Untertitel:</span></strong>') + 1:]
        ...
        if video['episode']:
            for folge_zeile in range(0, len(folge), 1):
                if int(folge[folge_zeile]) == int(video['episode']):
                    addLink("Staffel " + video['season'] + " Folge " + video['episode'] + " " + untertitel_release[
                        folge_zeile] + " ( " + untertitel_qualitaet[folge_zeile] + " ) ", untertitel_link[folge_zeile],
                            "download", duration="", desc="", genre='', lang=lang_array[folge_zeile])
                    gefunden = 1
    ...
    if '<span style="font-size: 14pt"><strong><span style="text-decoration: underline">Deutsche Untertitel:</span></strong>' in content:
        contentDE = content[content.find(
            '<span style="font-size: 14pt"><strong><span style="text-decoration: underline">Deutsche Untertitel:</span></strong>') + 1:]
        contentDE = contentDE[:contentDE.find(
            '<span style="font-size: 14pt"><strong><span style="text-decoration: underline">Englische Untertitel:</span></strong>')]
        folgede, untertitel_qualitaetde, untertitel_releasede, untertitel_linkde, lang_arrayde = get_content_old(
            contentDE, "en")
        if video['episode']:
            for folge_zeile in range(0, len(folge), 1):
                if int(folge[folge_zeile]) == int(video['episode']):
    ...
    if gefunden == 0:
        for folgenr in range(0, len(folge), 1):
            addLink(video['season'] + " Folge " + untertitel_release[folgenr] + " ( " + untertitel_qualitaet[
                folgenr] + " ) ", untertitel_link[folgenr], "download", "", duration="", desc="", genre='',
                    lang=lang_array[folgenr])
        for folgenr in range(0, len(folge), 1):
            addLink(video['season'] + " Folge " + folge[folgenr] + " " + untertitel_release[folgenr] + " ( " +
                    untertitel_qualitaet[folgenr] + " ) ", untertitel_link[folgenr], "download", "", duration="",
                    desc="", genre='', lang=lang_array[folgenr])
