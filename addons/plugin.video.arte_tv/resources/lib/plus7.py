...
def list_videos(
    ...
    url = 'http://www.arte.tv/papi/tvguide/videos/plus7/program/%s/%s/%s/%s/%i/%s/%i/%i/DE_FR.json' % (language_code, detail_level, category, cluster, recommended, sort, limit, offset)
    try: 
    ...
    if len(videos) == limit: # most likely this page isn't the limit...
        link = '%s?mode=list-videos&page=%i&category=%s&cluster=%s&sort=%s&recommended=%i' % (sys.argv[0], current_page + 1, category, cluster, sort, recommended)
        item = xbmcgui.ListItem('[B][COLOR fffa481c]%s (%i)[/COLOR][/B]' % (translate(30012), current_page + 2), iconImage='')
        xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=link, listitem=item, isFolder=True)
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
    return item_added