...
def get_stream_url(video_id):
	...
	debug("(get_stream_url) ### Xml_url : "+xml_url+" ###")
	try:
		xml_data = getUrl(xml_url)
		debug("(get_stream_url) ### Xml_data : "+xml_data+" ###")
		if '<live>true</live>' in xml_data:
			if not ('<islive>true</islive>' in xml_data):
				debug("(get_stream_url) ### LIVE-URL = NO--islive--Found ###")
				return False
		regex_url = '<url>(.+?)</url>'
		...
		regex_url_video = 'url="([^"]+?)"'
		video_url = re.findall(regex_url_video, xml_data_video, re.DOTALL)[0].replace('&amp;', '&')
		debug("(get_stream_url) ### Video_url : "+video_url+" ###")
		try:
			regex_auth_video = 'auth="([^"]+?)"'
			token_url = re.findall(regex_auth_video, xml_data_video, re.DOTALL)[0].replace('&amp;', '&')
			debug("(get_stream_url) ### Token_url : "+token_url+" ###")
		except: pass
	except: return False
	if '/dfb_live' in video_url and video_url.endswith('.m3u8') and token_url != "":
		debug("(get_stream_url) ### LIVE-URL : "+video_url+'?hdnea='+token_url+" ###")
		return video_url+'?hdnea='+token_url
	elif '/dfb_live' in video_url and video_url.endswith('.m3u8') and token_url == "":
		debug("(get_stream_url) ### LIVE-URL = NO--Token--Found ###")
		return False
	startURL = video_url[:video_url.find('_,')+2]
	if token_url != "": endURL = '?hdnea='+token_url
	else: endURL = ""
